...
function SunWenQR ( customize ) {
    ...
    // add in final mask/ecclevel bytes
        y = SunWenQR.fmtword[t + ((ecclevel - 1) << 3)];
        // low byte
        for (k = 0; k < 8; k++, y >>= 1)
            if (y & 1) {
...
SunWenQR.prototype = {
    ...
    canvasWrite : function(qf, width) {
        ...
        if(!this.qrc) {
            this.qrc = document.getElementById('code').getContext('2d');
    ...
    //getContext获取参canvas的2d对象、
    getContext: function(domElement) {

        //不能获取2d对象时，返回错误。（少数浏览器不支持canvas 2d 对象）
